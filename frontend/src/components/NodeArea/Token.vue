<template>
  <div :id="tokenId" :style="backgroundColorStyle" class="rounded mr-1 relative pt-2 mt-2 min-w-7" @mouseover="emitHoverEvent">
    <span class="token-index absolute top-0 left-1 text-xs" :data-token-id="tokenId">{{ index }}.</span>
    <span class="token pl-3" :data-token-id="tokenId">{{ text.replace(" ", "_").replace('\n', '\\n') }}</span>
  </div>
</template>
